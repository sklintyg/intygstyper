...
package se.inera.certificate.modules.lisu.rest;
...
import se.inera.certificate.modules.lisu.model.converter.*;
...
public class LisuModuleApi implements ModuleApi {

    private static final Logger LOG = LoggerFactory.getLogger(LisuModuleApi.class);
    ...
    private RegisterCertificateValidator validator = new RegisterCertificateValidator("lisu.sch");
    ...
    public String createNewInternalFromTemplate(CreateDraftCopyHolder draftCertificateHolder, String template)
            throws ModuleException {
        try {
            LisuUtlatande internal = getInternal(template);
    ...
    public void registerCertificate(String internalModel, String logicalAddress) throws ModuleException {
        ...
        try {
            request = InternalToTransport.convert(objectMapper.readValue(internalModel, LisuUtlatande.class));
    ...
    public Utlatande getUtlatandeFromJson(String utlatandeJson) throws IOException {
        return objectMapper.readValue(utlatandeJson, LisuUtlatande.class);
    }
    ...
    private CertificateResponse convert(GetCertificateResponseType response) throws ModuleException {
        try {
            LisuUtlatande utlatande = TransportToInternal.convert(response.getIntyg());
    ...
    private LisuUtlatande getInternal(String internalModel)
            throws ModuleException {

        try {
            LisuUtlatande utlatande = objectMapper.readValue(internalModel,
                    LisuUtlatande.class);
    ...
    private String updateInternal(String internalModel, HoSPersonal hosPerson, LocalDateTime signingDate)
            throws ModuleException {
        try {
            LisuUtlatande intyg = getInternal(internalModel);
    ...
    private String toInternalModelResponse(LisuUtlatande internalModel) throws ModuleException {
        try {
    ...
    public Intyg getIntygFromUtlatande(Utlatande utlatande) throws ModuleException {
        try {
            return UtlatandeToIntyg.convert((LisuUtlatande) utlatande);
        } catch (Exception e) {
