...
public class LuaenaModuleApi implements ModuleApi {
    ...
    private LuaenaUtlatande getInternal(String internalModel)
            throws ModuleException {

        try {
            LuaenaUtlatande utlatande = objectMapper.readValue(internalModel,
                    LuaenaUtlatande.class);
    ...
    private String toInternalModelResponse(LuaenaUtlatande internalModel) throws ModuleException {
        try {
    ...
    public String createNewInternalFromTemplate(CreateDraftCopyHolder draftCertificateHolder, String template)
            throws ModuleException {
        try {
            LuaenaUtlatande internal = getInternal(template);
    ...
    public void registerCertificate(String internalModel, String logicalAddress) throws ModuleException {
        ...
        try {
            request = InternalToTransport.convert(objectMapper.readValue(internalModel, LuaenaUtlatande.class));
        } catch (ConverterException | IOException e) {
            LOG.error("Failed to convert to transport format during register luae_na", e);
            throw new ExternalServiceCallException("Failed to convert to transport format during register luae_na", e);
        }
    ...
    private CertificateResponse convert(GetCertificateResponseType response) throws ModuleException {
        try {
            LuaenaUtlatande utlatande = TransportToInternal.convert(response.getIntyg());
    ...
    private String updateInternal(String internalModel, HoSPersonal hosPerson, LocalDateTime signingDate)
            throws ModuleException {
        try {
            LuaenaUtlatande intyg = getInternal(internalModel);
    ...
    public Utlatande getUtlatandeFromJson(String utlatandeJson) throws IOException {
        return objectMapper.readValue(utlatandeJson, LuaenaUtlatande.class);
    }
    ...
    public Intyg getIntygFromUtlatande(Utlatande utlatande) throws ModuleException {
        try {
            return UtlatandeToIntyg.convert((LuaenaUtlatande) utlatande);
        } catch (Exception e) {
