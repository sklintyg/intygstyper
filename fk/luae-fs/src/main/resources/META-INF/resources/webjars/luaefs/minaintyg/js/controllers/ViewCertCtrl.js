angular.module('luaefs').controller('luaefs.ViewCertCtrl',
    [ '$location', '$log', '$rootScope', '$stateParams', '$scope', 'common.IntygListService',
        ...
        function($location, $log, $rootScope, $stateParams, $scope, listCertService, certificateService, dialogService,
            ...
            $scope.send = function() {
                $location.path('/luaefs/recipients').search({ module: 'luaefs', defaultRecipient: 'FK'});
            };
            ...
            $scope.archiveSelected = function() {
                ...
                listCertService.archiveCertificate(item, function(fromServer, oldItem) {
                    ...
                    if (fromServer !== null) {
                        ...
                        // show error view
                        $location.path('/luaefs/fel/couldnotarchivecert');
                    }
                });
            };
            ...
            $scope.showStatusHistory = function() {
                $location.path('/luaefs/statushistory');
            };

            $scope.backToViewCertificate = function() {
                $location.path('/luaefs/view/' + $stateParams.certificateId);
            };

            // expose calculated static link for pdf download
            $scope.downloadAsPdfLink = '/moduleapi/certificate/' + 'luaefs' + '/' + $stateParams.certificateId + '/pdf';

            certificateService.getCertificate('luaefs', $stateParams.certificateId, function(result) {
