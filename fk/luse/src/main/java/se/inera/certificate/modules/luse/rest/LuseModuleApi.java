...
package se.inera.certificate.modules.luse.rest;
...
import se.inera.certificate.modules.luse.model.converter.*;
import se.inera.certificate.modules.luse.model.internal.LuseUtlatande;
import se.inera.certificate.modules.luse.validator.InternalDraftValidator;
...
public class LuseModuleApi implements ModuleApi {
    ...
    public String createNewInternalFromTemplate(CreateDraftCopyHolder draftCertificateHolder, String template)
            throws ModuleException {
        try {
            LuseUtlatande internal = getInternal(template);
    ...
    public void registerCertificate(String internalModel, String logicalAddress) throws ModuleException {
        ...
        try {
            request = InternalToTransport.convert(objectMapper.readValue(internalModel, LuseUtlatande.class));
        } catch (ConverterException | IOException e) {
            LOG.error("Failed to convert to transport format during register luse", e);
            throw new ExternalServiceCallException("Failed to convert to transport format during register luse", e);
        }
    ...
    public Utlatande getUtlatandeFromJson(String utlatandeJson) throws IOException {
        return objectMapper.readValue(utlatandeJson, LuseUtlatande.class);
    }
    ...
    private CertificateResponse convert(GetCertificateResponseType response) throws ModuleException {
        try {
            LuseUtlatande utlatande = TransportToInternal.convert(response.getIntyg());
    ...
    private LuseUtlatande getInternal(String internalModel)
            throws ModuleException {

        try {
            LuseUtlatande utlatande = objectMapper.readValue(internalModel,
                    LuseUtlatande.class);
    ...
    private String updateInternal(String internalModel, HoSPersonal hosPerson, LocalDateTime signingDate)
            throws ModuleException {
        try {
            LuseUtlatande intyg = getInternal(internalModel);
    ...
    private String toInternalModelResponse(LuseUtlatande internalModel) throws ModuleException {
        try {
    ...
    public Intyg getIntygFromUtlatande(Utlatande utlatande) throws ModuleException {
        try {
            return UtlatandeToIntyg.convert((LuseUtlatande) utlatande);
        } catch (Exception e) {
