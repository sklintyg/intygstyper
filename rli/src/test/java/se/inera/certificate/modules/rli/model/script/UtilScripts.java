...
public class UtilScripts {
	...
	private static se.inera.certificate.modules.rli.model.converters.TransportToExternalConverterImpl transportConverter =
			new se.inera.certificate.modules.rli.model.converters.TransportToExternalConverterImpl();
	 ...
	 * 
	 * @param filename The name of the XML to be converted
	 */
	private static void buildExternalFromFile(String filename){
		...
		try{
			utlatande = JAXB.unmarshal(new ClassPathResource(filename).getInputStream(), se.inera.certificate.common.v1.Utlatande.class);
