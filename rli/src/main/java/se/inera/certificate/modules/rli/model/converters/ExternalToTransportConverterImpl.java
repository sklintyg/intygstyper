...
public class ExternalToTransportConverterImpl implements
		ExternalToTransportConverter {
	...
	@Override
	public Utlatande externalToTransport(
	...
	List<RekommendationType> convertRekommendationer(
			List<Rekommendation> source) {
	...
	private Arrangemang convertArrangemang(
			se.inera.certificate.modules.rli.model.external.Arrangemang source) {
		if (source == null){
	...
	private PatientRelationType convertPatientRelation(PatientRelation source) {
		...
		patientRelationType.setAdress(source.getAdresses().toString());
		...
		patientRelationType.getEfternamns().addAll(source.getEfternamns());
