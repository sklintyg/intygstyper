define([ 'angular', 'webjars/common/webcert/js/services/CertificateService', 'webjars/common/webcert/js/services/ManageCertView',
    ...
    angular.module(moduleName, [ CertificateService, ManageCertView, User ]).controller(
            moduleName,
            [ '$anchorScroll', '$location', '$scope', CertificateService, ManageCertView, User,
                        ...
                        $scope.$watch('cert', function() {
                            if (!$scope.cert || (!$scope.cert.syn && !$scope.cert.horselBalans
                                 ...
                                 return;
                                    ...
                                    || $scope.cert.horselBalans.svartUppfattaSamtal4Meter == true || $scope.cert.funktionsnedsattning.funktionsnedsattning == true
                                    || $scope.cert.funktionsnedsattning.otillrackligRorelseformaga == true || $scope.cert.hjartKarl.hjartKarlSjukdom == true
                                    || $scope.cert.hjartKarl.hjarnskadaEfterTrauma == true || $scope.cert.hjartKarl.riskfaktorerStroke == true
                                    || $scope.cert.diabetes.harDiabetes == true || $scope.cert.neurologi.neurologiskSjukdom == true
                                    || $scope.cert.medvetandestorning.medvetandestorning == true || $scope.cert.njurar.nedsattNjurfunktion == true
                                    || $scope.cert.kognitivt.sviktandeKognitivFunktion == true || $scope.cert.somnVakenhet.teckenSomnstorningar == true
                                    || $scope.cert.narkotikaLakemedel.teckenMissbruk == true || $scope.cert.narkotikaLakemedel.foremalForVardinsats == true
                                    || $scope.cert.narkotikaLakemedel.provtagningBehovs == true || $scope.cert.narkotikaLakemedel.lakarordineratLakemedelsbruk
                                    || $scope.cert.psykiskt.psykiskSjukdom == true || $scope.cert.utvecklingsstorning.psykiskUtvecklingsstorning == true
                                    || $scope.cert.utvecklingsstorning.harSyndrom == true || $scope.cert.sjukhusvard.sjukhusEllerLakarkontakt == true
