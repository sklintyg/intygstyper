...
public class FullExternalToInternalConverterTest {
    ...
    public void testConvertUtlatande() throws Exception {
        ...
        ${package}.${artifactId}.model.external.Utlatande extUtlatande = readUtlatandeFromJsonFile("/${artifactId}-example-3.json");
    ...
    private ${package}.${artifactId}.model.external.Utlatande readUtlatandeFromJsonFile(String fileName) {

        ${package}.${artifactId}.model.external.Utlatande extUtlatande = null;

        InputStream src = this.getClass().getResourceAsStream(fileName);

        try {
            CustomObjectMapper mapper = new CustomObjectMapper();
            extUtlatande = mapper.readValue(src, ${package}.${artifactId}.model.external.Utlatande.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
